...
class SSIM_Matlab:
    def __init__(self, window_size=11, size_average=True, full=False, val_range=None):
        self.window_size = window_size
        self.size_average = size_average
        self.val_range = val_range
        self.full = full
    ...
    def calc(self, img1, img2):
        ...
        if self.window is None or self.real_size != real_size:
            self.real_size = real_size
            self.window = create_window_3d(real_size, channel=1).to(img1.device)
        return ssim_matlab(
            img1,
            img2,
            window=self.window,
